...
Задачи Phase 2+ (не расписаны заранее — добавляются в `TASKS.md` по мере продвижения). Дополнительные сущности из `22_DATA_MODEL.md` §54 (Baseline, Hypothesis, RecommendationReason, Action, Outcome) и `ExternalAccount` — по мере практической необходимости; обязательная сверка перед Phase 7/8, см. `DECISIONS.md` D-0008.
