...
## Текущая фаза
**Phase 0 — Project Foundation: завершена** (Task 0.1–0.6). **Phase 1 — Database Foundation: завершена** (Task 1.1–1.2), кроме критерия «connected account» (сознательно отложен до Phase 3/Task 3.0). Критерий Task 0.6 «deploy в staging» сознательно отложен — продуктовое решение Olga (`DECISIONS.md`, D-0007), не блокирует Phase 1.
...
## В работе
Ничего технического — Phase 1 полностью завершена и верифицирована. (Backlog, не блокирует Phase 2+): реальные staging/production ресурсы (Vercel, Neon, Upstash, R2) и branch protection для `main` — см. Backlog в `TASKS.md`, D-0007; расширение схемы (Baseline/Hypothesis/RecommendationReason/Action/Outcome, ExternalAccount) — по мере необходимости, см. `DECISIONS.md` D-0008.
...
Phase 2 — Authentication начата: Task 2.1 (Auth.js — техническая основа: schema + конфигурация) сформулирована в `TASKS.md`, ещё не реализована. Остальные задачи Phase 2 не расписаны заранее. Дополнительные сущности из `22_DATA_MODEL.md` §54 (Baseline, Hypothesis, RecommendationReason, Action, Outcome) и `ExternalAccount` — по мере практической необходимости; обязательная сверка перед Phase 7/8, см. `DECISIONS.md` D-0008.
