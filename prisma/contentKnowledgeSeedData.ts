 ...
 * Раздела 3 (стр. 8–11, Task 9.6), стр. 11–15 (Task 9.7 — концепция
 * позиционирования, формат микро-сериала, принципы влога) и стр. 16–35
 * (Task 9.8 — функция Series, функции заголовка, тренды середины года,
 * система планирования дня, обновление каруселей, шаблоны карусели,
 * Trial Reels, 4 новых hook_template, организация телефона; 2 новые
 * категории — `workflow_system`, `content_format_template`).
 *
 ...
 * content ideas", "vlog storylines", "non-selfish vlog titles") — привязаны
 * к конкретной нише/персоне, не переиспользуемый приём. Также НЕ включены
 * (стр. 16–35, Task 9.8, честно исключённое по другой причине — не
 * нишевая привязка, а дубликат/не по теме): таблица "I-Centric → We-Centric"
 * (дублирует уже загруженное `headline_rule` Правило 1); тренд
 * "Micro-Series" из MID YEAR TREND SPOTTING (дублирует уже загруженный
 * формат 30-дневного микро-сериала, Task 9.7); хук "Reverse Psychology"
 * из UNIQUE HOOKS (дублирует уже загруженный хук среды); "hook schedule
 * for this week" (прикладной пример уже загруженных типов хуков на
 * конкретную, не универсальную неделю, не новый шаблон); "Digital Diet"
 * (дублирует метафору "контентных шор" из уже загруженного фундамента
 * стратегии); бизнес-автоматизация через Gusto/EIN/payroll (не про
 * создание контента, вне области ContentKnowledge); "Экран 3: business
 * studio" из организации телефона (конкретные банковские/бизнес-приложения
 * одного автора, не переиспользуемый принцип). Стр. 36–130 (95 страниц)
 * СОЗНАТЕЛЬНО не загружены этим шагом — материал для следующих узких
 * шагов.
 *
