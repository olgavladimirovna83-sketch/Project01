 ...
 * Загружены: Раздел 1 (Недельная система хуков, Task 9.3) и Раздел 2
 * (Тренды монтажа, Task 9.6 — целиком, короткий раздел, 3 приёма).
 *
 * Раздел 3 (Task 9.6) — загружена ТОЛЬКО буквально одноимённая часть
 * (стр. 8–11: сравнение алгоритмов платформ, "что публиковать" на
 * TikTok/Instagram/YouTube, схема переиспользования контента между
 * платформами) — именно то, что называется "Стратегия диверсификации
 * контента по платформам и его переиспользование" в заголовке раздела.
 * ЧЕСТНО ЗАФИКСИРОВАНО: после стр. 11 заголовок "Раздел 3" в документе
 * не меняется до самого конца (стр. 130), но содержание фактически
 * перестаёт быть тем же разделом — это расшифровки множества разных
 * отдельных видео (идеи для "Comfort Creator", микро-сериалы, сюжеты
 * влогов, типографика заголовков, шрифты, оформление рабочего
 * пространства, примеры био, CTA-стратегии и т.д.), не одна связная
 * тема. Дальнейшие 119 страниц СОЗНАТЕЛЬНО не загружены этим шагом —
 * не "остаток раздела 3" одним куском, а материал для нескольких
 * будущих узких шагов по мере того, как Olga решит их пополнять
 * (см. DECISIONS.md, новая запись).
 *
