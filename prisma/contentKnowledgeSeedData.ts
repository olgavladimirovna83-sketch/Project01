 ...
 *
 * "Полное руководство.pdf" (130 страниц) загружалось по узким шагам —
 * Task 9.3 (Раздел 1, стр. 1–6), Task 9.6 (Раздел 2 целиком + буквально
 * одноимённая часть Раздела 3, стр. 7–11), Task 9.7 (стр. 11–15),
 * Task 9.8 (стр. 16–35), Task 9.9 (стр. 36–62), Task 9.10 (стр. 63–82),
 * Task 9.11 (стр. 83–102), Task 9.12 (стр. 103–130 — КОНЕЦ документа,
 * загрузка полностью завершена). Подробный список записей и обоснование
 * каждого шага — в `TASKS.md`, полная история включений/исключений — в
 * `CHANGELOG.md`, YELLOW-решения — в `DECISIONS.md` (D-0030/D-0032/D-0033).
 *
 * ДВЕ СИСТЕМАТИЧЕСКИЕ ПРИЧИНЫ ИСКЛЮЧЕНИЯ на каждом шаге (см. DECISIONS.md
 * D-0032): (1) нишевые/сезонные банки идей и заголовков — привязаны к
 * конкретной нише/персоне/месяцу (Comfort Creator, Unique Signature
 * Series, майские/мартовские сериалы и т.п.), не переиспользуемый приём,
 * тот же принцип, что исключение сезонных идей доп. файла (D-0030 п. 6);
 * (2) прямые дубликаты уже загруженного — документ пересказывает одни и
 * те же техники (особенно хуки и "фирменный стиль") много раз с разными
 * примерами под разными названиями; при обнаружении дубликата загружается
 * только первое вхождение, остальные явно пропускаются, не молча.
 * Заголовок "Раздел 3" в документе не меняется до самого конца (стр. 130),
 * но реально это расшифровки множества разных отдельных видео, не одна
 * тема — отсюда и деление на узкие шаги вместо одной большой загрузки.
 *
...
export const entries: SeedEntry[] = [
  ...
  // --- из "Полное руководство.pdf": Раздел 3, стр. 103–122 (Task 9.12) ---
  {
    categories: ['content_strategy'],
    title: 'Матрица оценки серии (Signature Series Matrix) — 4 критерия',
    content:
      '4 критерия для оценки, стоит ли развивать концепцию серии: Repeatable & Binge-worthy — можно ли производить последовательно, захочет ли аудитория смотреть «запоем»; Brand Deal Potential — смогут ли бренды-мечты увидеть себя в серии; Emotional Impact — задействует ли серия реальные эмоции, помогающие построить идеальное сообщество; Embedded Value — содержит ли серия микро-обучение и практическую пользу.',
    source: GUIDE,
    sourceSection: 'стр. 103, Signature Series Matrix',
  },
  {
    categories: ['workflow_system'],
    title: 'Техническая структура дня батчинга контента',
    content: [
      'Часовое расписание одного дня съёмки (пример): утро — публикация еженедельного Q&A, затем брейнсторм идей, планирование расписания публикаций, далее блок съёмки, затем блок монтажа.',
      'Карточка отдельного видео в планировщике содержит: тип контента (формат+хронометраж), статус производства (не начато / нужен сценарий / нужна съёмка / нужен монтаж / готово к публикации / опубликовано), опорные пункты хука и подписи вместо полного суфлёра-сценария.',
      'Оборудование: естественный свет как основа, усиленный двумя мощными кольцевыми лампами и одной мини-кольцевой; мини-микрофон для качественного звука. Процесс: смена наряда для каждого видео; текст не читается по сценарию, только опорные буллиты. Главный фокус в кадре — хук, свет, ценность. Объём за сессию — от 3 до 8 видео, часть снимается специально только для тестовых Trial Reel, не для основной ленты.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 104-106, How To Content Batch',
  },
  {
    categories: ['content_strategy'],
    title: 'Messy-контент как «нарушитель паттерна» восприятия ленты',
    content:
      'Механизм, почему неидеальный (messy) контент работает лучше отполированного: аудитория привыкла к идеальной картинке в ленте, поэтому реальные, неидеальные кадры резко выделяются и ломают привычный сценарий прокрутки (pattern disruptor) — именно контраст с ожидаемым делает такой контент заметным, а не сам факт «неидеальности» сам по себе.',
    source: GUIDE,
    sourceSection: 'стр. 108-109, Pattern Disruptor',
  },
  {
    categories: ['content_strategy'],
    title: 'Стратегия первых трёх постов нового аккаунта',
    content:
      'Формула для новых авторов: Post #1 = Attention (внимание) — фирменная серия или микро-серия, сразу дающая чёткое понимание, что подписчик получит; Post #2 = Trust (доверие) — контент «против течения», демонстрирующий отличие от типичных авторов в нише; Post #3 = Following (подписка) — максимально полезный контент, конвертирующий зрителя в подписчика через щедрую отдачу пользы. Итог: первый пост — внимание, второй — доверие, третий — подписка.',
    source: GUIDE,
    sourceSection: 'стр. 114-116, Getting Over the Starting Scaries',
  },
  {
    categories: ['platform_strategy'],
    title: 'Google Search Profile как «домашняя база» цифрового присутствия',
    content:
      'Инструмент, консолидирующий все соцсети и последний контент автора в одном настраиваемом поисковом окне (отдельно от Knowledge Panel, но связан с ней) — можно указать ссылку на него как «home base» в шапках других профилей. Преимущества: повышает находимость в Google, повышает доверие за счёт централизованного отполированного профиля, помогает брендам быстро понять, кто автор и какой контент создаёт. Требования на момент публикации видео: 100k+ подписчиков в Instagram/YouTube/X, либо 300k для TikTok-only авторов.',
    source: GUIDE,
    sourceSection: 'стр. 117, Google Search Profiles',
  },
  {
    categories: ['platform_strategy'],
    title: 'Сдвиг алгоритма Instagram: с размера аудитории на качество удержания',
    content:
      'Старый алгоритм Instagram отдавал приоритет количеству подписчиков и опирался на существующую аудиторию автора. Новый алгоритм отдаёт приоритет самому контенту — вознаграждает высокий retention rate, комментарии, сохранения и репосты, независимо от размера аудитории на старте.',
    source: GUIDE,
    sourceSection: 'стр. 117-118, Instagram Updates That Actually Affect You',
  },
  {
    categories: ['posting_schedule'],
    title: 'Недельный календарь тем для Stories',
    content:
      'Понедельник — подборка вдохновения (Inspo roundup); Вторник — напоминания (Reminders); Среда — закулисье рабочего процесса (Workflow Wednesday, BTS); Четверг — календарь хобби (Hobby Calendar); Пятница — 24-часовой перерыв без Stories (сброс); Суббота — сессия цифрового шопинга/корзины (Digi Shopping Cart Sessions); Воскресенье — влог-сброс (Sunday Reset VLOG).',
    source: GUIDE,
    sourceSection: 'стр. 118, Instagram Story Weekly Schedule',
  },
  {
    categories: ['content_format_template'],
    title: 'Выбор и удержание 2-3 постоянных форматов контента',
    content:
      'Принцип: не пытаться делать все возможные форматы — выбрать 2-3 и придерживаться их для узнаваемости. Варианты форматов на выбор: talking head (разговор на камеру); карусель; закадровый голос (voiceover); B-roll с текстом на экране; зелёный экран + разговор на камеру.',
    source: GUIDE,
    sourceSection: 'стр. 119-120, Consistent hooks & format',
  },
  {
    categories: ['content_strategy'],
    title: 'Freebie вместо постоянных продаж',
    content: [
      'Принцип: вместо призыва к действию на продажу в каждом посте — предлагать бесплатный полезный материал в обмен на комментарий с ключевым словом, либо давать больше пользы прямо в подписи.',
      'Шаги создания freebie: (1) определить тему и суть материала; (2) наговорить/выгрузить всё, что знаешь по теме (голосовой инструмент); (3) дать AI структурировать выгруженные знания в готовый материал; (4) дать AI оформить результат в документ/страницу для раздачи.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 120, Freebies over constant selling',
  },
  {
    categories: ['content_strategy'],
    title: '5 способов «удвоить» успешный контент (Double Down)',
    content:
      'Когда часть контента сработала — 5 способов расширить её без придумывания с нуля: (1) Zoom in — взять одну маленькую часть исходного контента и углубиться только в неё; (2) Zoom out — показать более крупную систему за конкретным действием, не только сами шаги; (3) тот же контент, другой формат — карусель / закадровый голос вместо разговора на камеру / только текст на экране; (4) Reverse it — показать противоположное или то, что НЕ стоит делать; (5) сменить угол через шаблоны «почему» (why I do X this way), «раньше делал неправильно» (what I used to do wrong with X — мини-история), «непопулярное мнение» (unpopular opinion about X — одна сильная позиция, без демонстрации своей рутины).',
    source: GUIDE,
    sourceSection: 'стр. 120-121, Double down on what works',
  },
  {
    categories: ['content_strategy'],
    title: 'Нехватка идей — проблема обучения, не творческий блок',
    content: [
      'Принцип: если идеи закончились, дело не в «творческой сломанности», а в том, что автор перестал давать себе новое — новое обучение, новых людей для разговора, выход за пределы зоны комфорта.',
      'Источники идей: собственное мнение о своей нише (даже спорное); реакция на то, что сделал кто-то другой (друг, незнакомец, другой автор); разговоры (на работе, на встрече, в переписке); недавняя ошибка и то, что сделал бы иначе; вопрос, который люди повторно задают в директ/комментариях; то, что прочитал/послушал и что изменило взгляд; тренд или спор, происходящий в нише прямо сейчас; случайная записанная мысль.',
      'Вопросы для поиска своей «грани»: что я знаю такого, чего не знают многие в моей нише? какое у меня мнение, с которым согласны не все? о чём люди чаще всего просят у меня совета в директ? что я мог бы объяснить новичку меньше чем за минуту? что я узнал недавно, что изменило то, как я что-то делаю?',
      'Как «подпитывать» творчество: подкасты в «мёртвое» время (дорога, прогулка, посуда); чтение книг/рассылок в своей нише; подписка на людей ВНЕ своей ниши (часто даёт лучшие идеи); разговор с тем, кто знает больше (ментор, коллега по индустрии, даже переписка в директ); тестирование новых инструментов в нише; добавление одного повторяющегося блока времени в неделю специально под обучение.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 121-122, What no one is saying about creativity',
  },

  // --- из "Полное руководство.pdf": Раздел 3, стр. 122–130 (Task 9.12,
  // КОНЕЦ документа — загрузка "Полное руководство.pdf" полностью
  // завершена этим шагом) ---
  {
    categories: ['content_format_template'],
    title: '5 переиспользуемых структур серии (reset/лог обучения/обратный отсчёт/анти-привычка/дневник хобби)',
    content:
      'Пять generic структурных форматов серии, применимых к любой нише: (1) Quarterly reset series — регулярный «сброс» конкретной сферы жизни (гардероб, дом, фитнес, финансы и т.д.), одна серия = один аспект; (2) Learning log series — «чему меня научило X на этой неделе» (книга, случайное видео, разговор, неудача); (3) Countdown series — обратный отсчёт до конкретного события с еженедельными чек-инами подготовки («X недель до переезда/запуска/забега»); (4) Anti-habit series — «день/неделя/месяц без ___» (без телефона, без жалоб, без импульсивных покупок); (5) Hobby diary — «я впервые попробовал(а) X» с документированием процесса.',
    source: GUIDE,
    sourceSection: "стр. 122-124, Series ideas that aren't boring",
  },
  {
    categories: ['content_strategy'],
    title: 'UGC-контент для брендовых сделок: практические правила съёмки',
    content: [
      'Отличие UGC от инфлюенсерского контента — нужно понимать разницу, а не переносить обычный стиль повествования.',
      'Практика: научиться комфортно говорить на камеру; убирать длинные паузы при монтаже; держать быстрый темп; добавлять субтитры; использовать понятные хуки и чёткие призывы к действию.',
      'Источники вдохновения: библиотека рекламы Meta (Meta Ad Library) — смотреть, какую рекламу реально крутят любимые бренды; изучать Instagram интересующих брендов; обращать внимание на рекламу, которую таргетируют лично на тебя.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 124, UGC content creation',
  },
  {
    categories: ['content_strategy'],
    title: 'Портфолио и питчинг для брендовых сделок без большой аудитории',
    content: [
      'Ключевая идея: количество подписчиков НЕ определяет брендовые сделки — соцсети работают как портфолио. Пути построения личного бренда для брендов: постоянная серийная рубрика; чёткая точка зрения (мнения); стабильно показывать лицо; делиться закулисьем, не только результатом; узнаваемый визуальный стиль; отвечать на комментарии/директ.',
      'Чек-лист портфолио: 5-7 видео, лучшие ролики первыми, автор говорит в кадре (не только визуал), высокое качество (свет/звук/монтаж), смесь стилей (органика/рекламный формат/talking head/voiceover+broll), минимум 2 ролика в рекламном формате, раздел "обо мне" звучит лично, не сгенерировано ИИ.',
      'Питчинг: питчить малые и средние бренды, локальные бренды, кастомизировать каждый питч под конкретный бренд, не использовать шаблон, который "звучит как ChatGPT".',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 125-127, UGC портфолио и питчинг',
  },
  {
    categories: ['editing_style'],
    title: 'Монтаж вирусных voiceover-видео — техника нарезки и озвучки',
    content: [
      'Разнообразие планов — из одного простого действия (например, поход за кофе) снять несколько разных ракурсов/скоростей: заход в кафе, процесс приготовления, крупный план напитка, реакция на пробу, сам процесс питья.',
      'Быстрые склейки — резать клипы по 0.5-0.9 секунды для удержания внимания.',
      '«Говори то, чего не видно» — закадровый голос рассказывает не о том, что показано на видео, а о параллельной теме: как прошёл день, что происходит за кадром, недавнее осознание, цели/намерения, за что благодарен, сложности последнего времени.',
      'Динамичные субтитры — текстовые шаблоны с анимацией (например, эффект печатающегося курсора, слово за словом).',
      'Цепляющее интро — нарезка коротких превью-кадров (0.3 сек каждый) со всего видео + цепляющий заголовок со шрифтами/цветом/стикерами в начале.',
      'Метки ключевых моментов — таймкоды или заголовки на важных точках видео. Звуковые эффекты — короткие акцентные звуки на действиях (щелчок, всплеск, звук клавиатуры и т.п.).',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 127-128, How girls edit viral voiceover videos',
  },
  {
    categories: ['content_strategy'],
    title: 'Три прямолинейных принципа построения личного бренда',
    content: [
      '(1) Привыкнуть видеть себя на камере — освоиться с собственным лицом на видео (в том числе без макияжа), проработать вопросы самооценки и уверенности ДО того, как аккаунт вырастет — иначе психологическая нагрузка выгорает автора уже после роста.',
      '(2) Показывать больше — делиться победами в бизнесе и творческими работами, давать аудитории за что «болеть»; при правильной подаче это не читается как хвастовство.',
      '(3) Быть «раздражающе повторяющимся» — не бояться повторять свои фирменные фразы, форматы серий и узнаваемые элементы; если бренд/контент нельзя описать одним предложением, нужно повторять катч-фразы/форматы чаще, чтобы запомниться в море похожих авторов.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 128, Unhinged Social Media Advice',
  },
  {
    categories: ['content_format_template'],
    title: 'Binge-Watching Blueprint — баланс предсказуемости и предвкушения в структуре серии',
    content: [
      'Принцип: успешная серия балансирует между предсказуемостью (зритель знает общую структуру/формат) и предвкушением (не знает точно, что случится дальше) — человеческий мозг любит повторение и предсказуемость, устойчивая структура помогает аудитории легко запоминать и ждать контент.',
      'Примеры структур эпизода: (1) яркий момент-затравка → интро → начало истории; (2) начало → середина → конец; (3) клиффхэнгер-концовка → начало → середина → конец.',
      'Референс из индустрии реалити-ТВ (например, формат Keeping Up with the Kardashians) — вдохновляться структурой реалити-шоу, а не форматами других блогеров, и повторять одну и ту же формулу монтажа последовательно в каждом выпуске своей серии.',
    ].join('\n'),
    source: GUIDE,
    sourceSection: 'стр. 128-129, The Binge-Watching Blueprint',
  },
  {
    categories: ['content_strategy'],
    title: 'Личность как усилитель обычной идеи (YOU CODE your content)',
    content:
      'Принцип: чтобы контент запомнился, не нужна революционная идея — достаточно взять обычную, простую тему и сделать её уникальной через свой личный стиль общения. Демонстрация на одной и той же теме (поход за матчей) через разные типы подачи: язвительная (sassy — ирония про вкус и качество места), дерзкая/неформальная (unhinged — личный, почти "инсайдерский" комментарий), аналитическая (структурированное сравнение по критериям: цвет/вкус/текстура/запах), хаотичная/провокационная (chaotic — категоричное заявление, спор с другими мнениями). Один и тот же контентный повод даёт совершенно разный результат в зависимости от того, через какую "личность"/тон его подать.',
    source: GUIDE,
    sourceSection: 'стр. 129-130, YOU CODE YOUR CONTENT',
  },
];
