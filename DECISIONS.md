...
## D-0008 — Scope Task 1.1: 10 core entities по 18_DATA_MODEL.md §45, не 18-таблиц MVP_DATABASE по 25_DATABASE_SCHEMA.md §62

**Дата:** 12 августа 2026
**Статус:** Принято (YELLOW — техническая интерпретация неоднозначно пересекающихся спецификаций)

**Контекст.** Task 1.1 в `TASKS.md` перечисляет 10 сущностей (User, Goal, Content, ContentFeature, PerformanceMetric, Pattern, Memory, Recommendation, UserDecision, Experiment) — это дословно список из `18_DATA_MODEL.md` §45 «Минимальная версия Data Model». При этом `25_DATABASE_SCHEMA.md` §62 MVP_DATABASE даёт более широкий список из 18 таблиц (включая `external_accounts`, `baselines`, `pattern_evidence`, `recommendation_candidates`, `recommendation_reasons`, `actions`, `outcomes`, `events`, но без отдельной таблицы под Experiment-гипотезы). Оба документа — поздняя волна (17–46), конфликта волн здесь нет; это два документа разного уровня детализации (18 — концептуальная модель, 25 — таблицы), не дублирующие друг друга по D-0002.

**Решение:** реализован ровно 10-сущностный список из `TASKS.md`/`18_DATA_MODEL.md` §45 — не более широкий список `25_DATABASE_SCHEMA.md` §62. Обоснование: явная формулировка задачи в `TASKS.md` — по иерархии CLAUDE.md §2 явно принятое продуктовое определение задачи весит выше, чем более широкий (но не обязательный для MVP, судя по формулировке «необязательно физически реализовывать все перечисленные таблицы») список в §62. Поля сущностей взяты из `25_DATABASE_SCHEMA.md` (наиболее конкретный источник по колонкам/связям/индексам) там, где это не противоречит 10-сущностному scope.
...
**Последствия:** более широкий набор таблиц (`ExternalAccount`, `Baseline`, `PatternEvidence`, `RecommendationCandidate`, `RecommendationReason`, `Action`, `Outcome`, `Hypothesis`, `Event` и т.д.) реализуется в последующих задачах Phase 1+, по мере того как в них появляется практическая необходимость (Recommendation Engine, Instagram sync). Это не пересмотр архитектуры — `25_DATABASE_SCHEMA.md` остаётся источником истины по полям этих таблиц, когда до них дойдёт очередь.
