...
**Дата:** 12 августа 2026 (research), дополнено 13 августа 2026 (живая проверка через реальный API и настоящий OAuth flow, Task 3.2)
**Статус:** Research завершён; ключевые находки подтверждены живыми запросами против реального Instagram API (не только документация)
...
> **Обновление 12 августа 2026 (после первой версии этого документа).** Olga лично прошла реальную авторизацию Instagram (не создание нового Meta developer-аккаунта/приложения — использован существующий authorization flow) и подтвердила, какие permissions реально предлагаются и какие она осознанно выбрала. Это первичный источник выше документации Meta по иерархии `CLAUDE.md` §2 («явно принятые продуктовые решения Olga» — приоритет 1) и **исправляет** вывод первой версии этого раздела, который предполагал, что доступа к insights достаточно через один `instagram_business_basic`. На практике консент-экран показывает insights как **отдельное, самостоятельно включаемое разрешение**, не часть `instagram_business_basic`.

**Подтверждено Olga (актуальный набор для этого продукта):**

| Permission | Статус | Комментарий |
|---|---|---|
| `instagram_business_basic` | **Разрешён (обязательный)** | Доступ к профилю и медиа — базовая идентификация аккаунта, без него не работает ничего остальное, включая refresh token |
| Insights / статистика аккаунта | **Разрешён** | На консент-экране показан как отдельное разрешение от `instagram_business_basic`. Официальная объединённая таблица permissions Meta (`developers.facebook.com/docs/permissions/`) на момент проверки перечисляет для этой функции только старый scope `instagram_manage_insights` (Facebook Login-эра); вторичные источники называют современный эквивалент для Business Login — `instagram_business_manage_insights`. Точное имя scope в актуальной документации Meta для Business Login указано непоследовательно (см. gap ниже) — но сам факт, что это отдельное разрешение, подтверждён напрямую Olga, это первичнее документации |
| `instagram_business_manage_comments` | **Отключён Olga** | Не запрашивается — управление комментариями не нужно продукту |
| `instagram_business_manage_messages` | **Отключён Olga** | Не запрашивается — direct-сообщения не нужны продукту |
| `instagram_business_content_publish` | **Отключён Olga** | Не запрашивается — продукт не публикует контент от лица пользователя |

**Вывод для MVP (исправлено):** минимальный набор — `instagram_business_basic` **плюс отдельное insights-разрешение** (не одно только `instagram_business_basic`, как предполагала первая версия этого документа). Comments/messages/content-publish осознанно не запрашиваются — подтверждено Olga, соответствует тому, что продукт делает (аналитика и рекомендации, не постинг/не переписка от лица пользователя). Точное каноническое имя scope для insights в Instagram Login-варианте стоит зафиксировать по факту, когда будет создано тестовое Meta-приложение (сам этот шаг — отдельный RED, ждёт Olga) — на консент-экране Meta он показывается человекочитаемым названием, не голым именем scope, так что финальное имя параметра для OAuth-запроса надо будет считать из реального Meta App Dashboard, а не только из документации.
...
### 4.1 Закрытый вопрос: dashboard-токен vs токен из настоящего OAuth-flow (Task 3.2, 13 августа 2026)

При реализации `exchangeForLongLivedToken` (Task 3.2) обмен на long-lived токен систематически падал с ошибкой `Session key invalid...` (`OAuthException`, `code: 452`, `error_subcode: 2207055`) — воспроизводилось на нескольких заведомо свежих токенах, сгенерированных вручную через кнопку **"Generate Token" в Meta App Dashboard**. `code: 452` — легаси-код `API_EC_SESSION_INVALID` из дореформенного Facebook REST API, подтверждено независимо (не выдумано, а найдено в справочниках старых кодов ошибок Facebook Platform) — сама формулировка сообщения об ошибке действительно унаследована от старой системы, как и подозревалось.

**Эмпирически подтверждено** (не гипотеза больше): реальный browser OAuth round trip — `getAuthorizationUrl` → согласие пользователя на консент-экране → `code` → `exchangeCodeForTokens` (который сам делает short-lived → long-lived обмен через `ig_exchange_token`) — прошёл **чисто**, с первой попытки, без единой ошибки. Полученный токен живёт **~60 дней** (`obtainedAt` → `expiresAt`, разница ровно соответствует документированному сроку long-lived токена).

**Вывод:** токены, выданные через "Generate Token" в Meta App Dashboard, и токены, полученные через штатный `authorization_code` OAuth flow, — это **разные по своей природе session-объекты** на стороне Meta, не взаимозаменяемые для `ig_exchange_token`. Дашборд-токен, судя по всему, не предназначен для повторного обмена через этот grant type (сама Meta, вероятно, уже выдаёт его в форме, close к финальной, минуя обычную short-lived-стадию) — попытка прогнать его через `ig_exchange_token` спотыкается о легаси-проверку `session key`, которая ожидает токен другой формы. Это объясняет ошибку полностью и снимает вопрос: **не баг в реализации `exchangeForLongLivedToken`/`refreshTokens`** (оба метода, наоборот, подтверждены рабочими — именно они и обработали успешный обмен в реальном flow).

**Практическое следствие:** реальные пользователи продукта всегда будут проходить именно через `getAuthorizationUrl` → `exchangeCodeForTokens` (настоящий OAuth flow), никогда через ручную кнопку в дашборде — то есть эта ошибка **не будет воспроизводиться в продакшене**. Она была артефактом конкретно ручного тестового способа получения токена на этапе разработки, не свойством самого API или нашего кода.
...
1. **Явный endpoint/механизм revocation** (что именно вызывает наше приложение, если пользователь хочет отключить аккаунт из нашего продукта, не через настройки Instagram) — не найден в открытой документации при этой проверке. Нужно проверить отдельно при реализации persistence-слоя (`ExternalAccount`), возможно через Meta Business SDK или `DELETE /{user-id}/permissions`-подобный endpoint по аналогии с классическим Facebook Graph API. **Всё ещё открыт.**
2. ~~Точное каноническое имя scope для insights в Business Login-варианте~~ — **закрыто (13 августа 2026).** Реальный OAuth authorize-запрос со `scope=instagram_business_basic,instagram_business_manage_insights` прошёл живую проверку через настоящий браузер: пользователь увидел ожидаемый консент-экран, согласился, `code` пришёл, обмен на токен прошёл чисто. Это не стопроцентное подтверждение самого имени строки (Meta теоретически могла бы молча проигнорировать нераспознанный scope, не отклонив запрос) — но раз итоговый токен реально получил доступ к insights endpoints (`getAccountInsights` подтверждён рабочим ещё в первой живой проверке Task 3.2), запрошенный набор permissions в связке точно достаточен на практике.
3. ~~"Session key invalid" на `exchangeForLongLivedToken`~~ — **закрыто (13 августа 2026), см. §4.1.** Не баг реализации и не свойство API — артефакт токенов, выданных вручную через "Generate Token" в Meta App Dashboard, а не через настоящий OAuth flow. В продакшене не воспроизводится, т.к. реальные пользователи проходят только через полноценный `authorization_code` flow.
4. **Полный список deprecated media-insights метрик** взят из нескольких источников (включая вторичные, не только `developers.facebook.com`) — таблица в §5 должна быть перепроверена по официальному API Reference непосредственно перед реализацией media-insights, если пройдёт много времени. **Всё ещё открыт** (не проверялся живым запросом — только `getAccountInsights` был протестирован вживую, не `getMediaInsights`).

Пункт 1 и пункт 4 не блокируют дальнейшую разработку — их стоит закрыть при непосредственной реализации соответствующего функционала (persistence и media-level insights), не раньше.
...
- Запрашиваемые permissions — подтверждено Olga через реальную авторизацию: `instagram_business_basic` (обязательный) + отдельное insights-разрешение. Comments/messages/content-publish — осознанно не запрашиваются.
...
- **Полный OAuth round trip живьём подтверждён (13 августа 2026, Task 3.2)**: `getAuthorizationUrl` → согласие на реальном консент-экране → `code` → `exchangeCodeForTokens` (short-lived → long-lived внутри) — прошёл чисто с первой попытки через настоящий браузер, итоговый токен живёт ~60 дней. `getAccountInsights` тоже подтверждён живым запросом (первая проверка Task 3.2) — реальные данные аккаунта пришли корректно. Не подтверждены живым запросом: `refreshTokens` (нужен токен ≥24ч, не проверялось), `getMediaInsights`/`listRecentMedia`, `disconnect` (endpoint не существует в реализации, см. §8 gap 1).
