...
2. **Точное каноническое имя scope для insights в Business Login-варианте** — частично закрыто: сам факт, что insights требует отдельного разрешения от `instagram_business_basic`, подтверждён напрямую Olga через реальную авторизацию (см. §3). Остаётся открытым только точное строковое имя параметра `scope` для OAuth-запроса (кандидат — `instagram_business_manage_insights`, но официальная сводная таблица permissions Meta на момент проверки называет для этой функции только старый `instagram_manage_insights`) — уточняется по факту при создании тестового Meta-приложения, не блокирует дальнейшее планирование.
